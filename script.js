...
// ✅ 메가 진화 배경 추가
const MEGA_BG_URL = "./mega_bg.png";
let megaBgImg = new Image();
megaBgImg.src = MEGA_BG_URL;
megaBgImg.crossOrigin = "anonymous";
let megaBgLoaded = false;
megaBgImg.onload = () => { megaBgLoaded = true; };
...
// ✅ 메가 / 원시 포켓몬 배경
if ((pokemonObj.Form === "메가" || pokemonObj.Form === "원시") && megaBgLoaded) {
  ctx.drawImage(megaBgImg, 0, 0, canvas.width, canvas.height);
}
...
if (
  !["자시안(검왕폼)", "자마젠타(방패왕폼)", "무한다이노"]
    .includes(pokemonObj.Pokemon_Name) &&
...
const maxFormSelect = document.getElementById("maxFormSelect");

function updateMaxDatalist() {

  maxDatalist.innerHTML = "";

  const selectedForm = maxFormSelect.value;

  dynamaxPokemonStats
    .filter(p => !isNaN(p.Dynamax_Boss))
    .filter(p =>
      selectedForm === "Gigantamax"
        ? p.Form === "거다이맥스"
        : p.Form !== "거다이맥스"
    )
    .forEach(p => {
...
// 선택 변경 시 갱신
maxFormSelect.addEventListener("change", updateMaxDatalist);
...
function calcExpectedDamage(boss, defender, skill) {
  ...
  // ⭐ Dynamax_Boss 값 변환
  const bossStar = boss.Dynamax_Boss;
  const starMap = {
    1: 0.15,
    2: 0.38,
    3: 0.5,
  ...
  const dynamaxBoss = starMap[bossStar] ?? 0.5;
...
tankCandidates.forEach(defender => {
  ...
  // ===============================
// 🔵 v3.2 추천 탱커 기대 데미지 계산 (스페셜 1개 대응)
// ===============================
...
// 🟢 스페셜 기술이 1개뿐인 경우
if (bossSkills.length === 1) {

  const skill = bossSkills[0];

  const singleTarget =
    calcExpectedDamage(pokemonObj, defender, skill) * 1.5;

  const multiTarget =
    calcExpectedDamage(pokemonObj, defender, skill);

  // 단일 / 전체 평균
  totalDamage = (singleTarget + multiTarget) / 2;
  caseCount = 1;

}

// 🔵 스페셜 기술이 2개 이상인 경우 (기존 로직 유지)
else if (bossSkills.length >= 2) {
...
const survivalHits = Math.ceil(
  ((effectiveHP + 13) * 0.77) / avgDamage
);
