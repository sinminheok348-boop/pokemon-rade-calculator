...
// 🔹 2단계: 탱커 후보 (조건 필터 강화)
const tankCandidates = dynamaxPokemonStats.filter(p => {

  // 1️⃣ Dynamax 가능
  if (p.Dynamax !== "O") return false;

  // 2️⃣ 무한다이맥스 제외
  if (p.Pokemon_Name === "무한다이맥스") return false;

  // 3️⃣ 다이맥스 / 거다이맥스 동시 존재 시 → 일반폼만 사용
  if (p.Form && p.Form !== "일반") return false;
