...
function buildRecommendationList(selectedDefender, weather, megaSelectValue) {
  ...
  // 🔹 다이맥스/거다이맥스 포함 + 중복 제거 처리
const tankCandidates = pokemonStats.filter(p =>
  p.Dynamax === "O"
);

const processedNames = new Set();

for (let p of tankCandidates) {

  // 🔹 같은 이름이 이미 처리되었다면 skip (거다이맥스 중복 방지)
  if (processedNames.has(p.Pokemon_Name)) continue;

  // 🔹 같은 이름 중 일반폼(다이맥스)을 우선 선택
  const sameNameAll = tankCandidates.filter(x =>
    x.Pokemon_Name === p.Pokemon_Name
  );

  const baseForm =
    sameNameAll.find(x => !x.Form || x.Form === "일반") ||
    sameNameAll[0];

  processedNames.add(baseForm.Pokemon_Name);

  const fastMoves = [...(baseForm.Fast_Moves || []), ...(baseForm.Elite_Fast_Moves || [])];
  const chargedMoves = [...(baseForm.Charged_Moves || []), ...(baseForm.Elite_Charged_Moves || [])];
...
const avgDamage = totalDamage / caseCount;
  ...
  const survivalHitsRaw = defender.HP / avgDamage;
