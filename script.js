...
result.querySelectorAll("canvas.type-icon").forEach(c => {
  ...
  // 🔹 1단계: 다이맥스 + 거다이맥스 모두 포함
const dynamaxAvailable = pokemonStats
  .filter(p =>
    (p.Dynamax === "O" || p.Form === "거다이맥스") &&
    p.Pokemon_Name !== "무한다이맥스"
  )
  // 🔹 같은 이름이 여러 폼일 경우 일반 폼 우선
  .reduce((acc, curr) => {

    const existing = acc.find(x => x.Pokemon_Name === curr.Pokemon_Name);

    if (!existing) {
      acc.push(curr);
    } else {
      // 이미 있는데, 기존이 거다이맥스이고 현재가 일반이면 교체
      if (existing.Form === "거다이맥스" && curr.Form !== "거다이맥스") {
