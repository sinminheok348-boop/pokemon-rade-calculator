오케이!! 그렇다면 해당 내용을 다른 코드의 수정은 최소화하면서 딱 그부분만 바꿔줘! 이때 마찬가지로 정확히 어디를 어떻게 수정해야할지 전후비교로 구체적이고 명확히 알려줘! 아래는 내 현재 script.js 전체 코드야!
...
const tankCandidates = dynamaxPokemonStats.filter(
  p =>
    p.Dynamax === "O" &&
    (!p.Form || p.Form === "일반")
);
...
if (bossSkills.length === 2) {

  const damage1 = calcExpectedDamage(pokemonObj, defender, bossSkills[0]);
  const damage2 = calcExpectedDamage(pokemonObj, defender, bossSkills[1]);

  // ✅ 단일+전체 평균 기대값 구조 적용
  expectedDamage = (damage1 + damage2) / 2;

} else if (bossSkills.length === 1) {

  expectedDamage = calcExpectedDamage(pokemonObj, defender, bossSkills[0]);
...
const survivalHitsRaw = defender.HP / expectedDamage;
