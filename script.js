...
result.querySelectorAll("canvas.type-icon").forEach(c => {
  ...
  // 🔹 1단계: 다이맥스 가능한 포켓몬 필터
  // 🔹 다이맥스 + 거다이맥스 모두 포함
const dynamaxAvailableRaw = pokemonStats
  .filter(p =>
    p.Dynamax === "O" &&
    p.Pokemon_Name !== "무한다이맥스"
  );

// 🔹 같은 이름이 여러 폼일 경우 → 일반(다이맥스) 우선 선택
const dynamaxAvailableMap = {};

dynamaxAvailableRaw.forEach(p => {

  const name = p.Pokemon_Name;

  // 아직 등록되지 않았으면 저장
  if (!dynamaxAvailableMap[name]) {
    dynamaxAvailableMap[name] = p;
  }

  // 이미 등록되어 있고, 기존이 거다이맥스이며
  // 현재가 일반 폼이면 교체
  else {
    const existing = dynamaxAvailableMap[name];
...
const dynamaxAvailable = Object.values(dynamaxAvailableMap);
