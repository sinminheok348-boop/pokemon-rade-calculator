...
// --- 변경: attackerForm 기반 기본 MS (폼만) ---
let baseMS = 1;
if (attackerForm === "그림자") {
  baseMS = 1.2;
} else if (attackerForm === "메가" || attackerForm === "원시") {
  baseMS = 1.1;
} else {
  baseMS = 1;
}

// --- 변경: 노말/스페셜별 MS 계산 (메가+자속 => 1.3) ---
let MS_normal = baseMS;
let MS_special = baseMS;
if ((attackerForm === "메가" || attackerForm === "원시") && attackerTypes.includes(normalType)) {
  MS_normal = 1.3;
}
if ((attackerForm === "메가" || attackerForm === "원시") && attackerTypes.includes(specialType)) {
  MS_special = 1.3;
}
...
// Normal DPS (노말 전용 보정 사용)
const normalDPS = 0.5 * normalPWR * attackerAtk * MS_normal * O_normal * typeMult_normal / (defenderDef * normalDuration);

// Special Damage (스페셜 전용 보정 사용)
const specialDamage = 0.5 * specialPWR * attackerAtk * MS_special * O_special * typeMult_special / defenderDef;
...
const typeChart = {
  ...
  "강철":{"노말":1,"불꽃":0.625,"물":0.625,"풀":1,"전기":0.625,"얼음":1.6,"격투":1,"독":1,"땅":1,"비행":1,"에스퍼":1,"벌레":1,"바위":1.6,"고스트":1,"드래곤":1,"악":1,"페어리":1.6},
