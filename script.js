...
const dynamaxAvailable = dynamaxPokemonStats
  ...
  .filter((p, index, arr) => {
    ...
    if (p.Form === "거다이맥스") {
      ...
      // 같은 이름의 일반 폼이 존재하면 제외
      const hasNormal = arr.some(other =>
        other.Pokemon_Name === p.Pokemon_Name &&
        (!other.Form || other.Form === "일반")
      );
